...
public class EventsSubscription
{
    ...
    public String getChallengeParameter(@RequestBody String body)
    {
        ...
        try {
            ...
            JSONTokener temp = new JSONTokener(parser.toString());
            JSONObject json = new JSONObject(temp);

             if(json.keySet().contains("challenge"))
                return json.get("challenge").toString();
             else
             {
                 if(json.keySet().contains("event"))
                 {
                     ...
                     if(eventObject.get("type").toString().equals("reaction_added"))
                     {
                         ...
                         jsonObject.put(
                                 "text",
                                 "Hi! I am StudentBot. Welcome to Code Your Future."
                                         + " Are you Student or a Volunteer....");
